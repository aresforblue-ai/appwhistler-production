        ...
        <section className="max-w-7xl mx-auto px-6 py-16">
          ...
          {error && (
            <div className="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-2xl p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-red-800 dark:text-red-200 mb-2">Unable to Connect to Server</h3>
              <p className="text-red-600 dark:text-red-300 mb-4">
                {error.message.includes('fetch') ?
                  'Could not connect to the backend server. Please make sure the server is running on port 5000.' :
                  error.message}
              </p>
              <button
                onClick={() => window.location.reload()}
                className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-full font-semibold transition-all"
              >
                Retry Connection
              </button>
            </div>
