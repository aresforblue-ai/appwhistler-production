...
const perUserRateLimiter = rateLimit({
  ...
  // Only use custom keyGenerator for authenticated users
  // Let express-rate-limit handle IP-based limiting with its default behavior
  keyGenerator: (req) => {
    if (req.user?.userId) {
      return `user:${req.user.userId}`;
    }
    // Return undefined to use default IP-based key generation (handles IPv6 properly)
    return undefined;
  },
